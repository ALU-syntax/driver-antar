...
import com.insoftbumdesku.driver.constants.VersionChecker;
import com.insoftbumdesku.driver.fragment.HomeFragment;
import com.insoftbumdesku.driver.fragment.OrderFragment;
...
import com.insoftbumdesku.driver.utils.MyLocationService;
import com.insoftbumdesku.driver.fragment.HistoryFragment;
import com.insoftbumdesku.driver.fragment.MessageFragment;
import com.insoftbumdesku.driver.fragment.ProfileFragment;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void cekberkas() {
        ...
        CekBerkasRequestJson param = new CekBerkasRequestJson();
        param.setId(loginuser.getId());
        driverservice.cekberkas(param).enqueue(new Callback<CekBerkasResponseJson>() {
            @Override
            public void onResponse(Call<CekBerkasResponseJson> call, Response<CekBerkasResponseJson> response) {
                if(response.isSuccessful()){
                    ...
                    if(resultcode.equalsIgnoreCase("00")) {
                        ...
                        if(status.equalsIgnoreCase("kosong")) {
                            Intent intent = new Intent(MainActivity.this, StnkActivity.class);
            ...
            @Override
            public void onFailure(Call<CekBerkasResponseJson> call, Throwable t) {
    ...
    private void cekwilayah() {
        ...
        CekWilayahRequestJson param = new CekWilayahRequestJson();
        param.setId(loginuser.getId());
        driverservice.cekwilayah(param).enqueue(new Callback<CekWilayahResponseJson>() {
            @Override
            public void onResponse(Call<CekWilayahResponseJson> call, Response<CekWilayahResponseJson> response) {
                if(response.isSuccessful()) {
                    String sukses = response.body().getStatus();
                    if(sukses.equalsIgnoreCase("kosong")) {
            ...
            @Override
            public void onFailure(Call<CekWilayahResponseJson> call, Throwable t) {
            ...
            public void onResponse(@NonNull Call<GetHomeResponseJson> call, @NonNull final Response<GetHomeResponseJson> response) {
                if (response.isSuccessful()) {
                    if (Objects.requireNonNull(response.body()).getMessage().equalsIgnoreCase("success")) {
                        ...
                        finish();
                        Toast.makeText(MainActivity.this, "Akun anda di nonaktifkan, silahkan kontak admin!", Toast.LENGTH_SHORT).show();
                    }
                }
            }
